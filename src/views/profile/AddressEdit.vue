...
<script>
  ...
  import {addAddress, editAddress, deleteAddress, getAddress, getAddressDetail} from "@/network/address";
          ...
          getAddressDetail(addressId).then(res=>{
            ...
            Object.entries(state.areaList.county_list).forEach(([id, text]) => {
              ...
              if (text == addressDetail.county) {
                // 找到区对应的几个省份
                const provinceIndex = tdist.getLev1().findIndex(item => item.id.substr(0, 2) == id.substr(0, 2))
                // 找到区对应的几个市区
                // eslint-disable-next-line no-unused-vars
                const cityItem = Object.entries(state.areaList.city_list).filter(([cityId, city]) => cityId.substr(0, 4) == id.substr(0, 4))[0]
                // 对比找到的省份和接口返回的省份是否相等，因为有一些区会重名
                if (tdist.getLev1()[provinceIndex].text == addressDetail.province && cityItem[1] == addressDetail.city) {
                  _areaCode = id
                }
              }
            })
